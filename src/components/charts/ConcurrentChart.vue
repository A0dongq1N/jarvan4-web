<template>
  <div class="chart-wrapper">
    <div class="chart-wrapper__header">
      <span class="chart-wrapper__dot" style="background:#1b855e" />
      <span class="chart-wrapper__title">{{ title ?? '并发用户数' }}</span>
    </div>
...
const option = computed(() => ({
  ...
  tooltip: {
    ...
    formatter: (params: any) => {
      const p = params[0]
      return `<span style="color:#9c9fa3;font-size:11px">${p.axisValue}</span><br/>并发数 <b style="color:#1b855e">${p.value}</b>`
    },
  },
  series: [{
    type: 'line',
    ...
    areaStyle: {
      color: {
        ...
        colorStops: [
          { offset: 0, color: 'rgba(27,133,94,0.18)' },
          { offset: 1, color: 'rgba(27,133,94,0)' },
        ],
      },
    },
  }],
